...
export function shortenText(ctx, text, maxWidth) {
	let shorten = false;
...
export function silhouette(ctx, x, y, width, height) {
	const data = ctx.getImageData(x, y, width, height);
	for (let i = 0; i < data.data.length; i += 4) {
		data.data[i] = 0;
		data.data[i + 1] = 0;
...
export function desaturate(ctx, level, x, y, width, height) {
	const data = ctx.getImageData(x, y, width, height);
...
export function distort(ctx, amplitude, x, y, width, height, strideLevel = 4) {
	const data = ctx.getImageData(x, y, width, height);
...
export function fishEye(ctx, level, x, y, width, height) {
...
export function pixelize(ctx, canvas, image, level, x, y, width, height) {
...
export function drawImageWithTint(ctx, image, color, x, y, width, height) {
...
export function centerImage(base, data) {
...
export function centerImagePart(data, maxWidth, maxHeight, widthOffset, heightOffest) {
	let { width, height } = data;
	if (width > maxWidth) {
...
export function generateErrorImage(error) {
